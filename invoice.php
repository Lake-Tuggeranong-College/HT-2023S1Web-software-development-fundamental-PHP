 ...
 * The invoices page has a number of use cases to satisfy:
  ...
  @var $conn
 */
//checks to see if the user is not logged in, if they aren't logged in redirects user to home page
...
// Produce a list of links of the Orders for the user.
        ...
        if ($_SESSION["AccessLevel"] == 1) {
            ...
            //reloads page when change is made to OPEN/CLOSED status
            $query = $conn->query("SELECT  Status FROM Orders  WHERE OrderNumber='$orderNumber'");
            $data = $query->fetchArray() ;
            $status = $data ["Status"];
            //changes status between OPEN and CLOSED
            if ($status == "OPEN") {
                echo "STATUS: OPEN";
